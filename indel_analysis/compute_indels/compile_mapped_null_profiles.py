...
if __name__ == '__main__':
	...
	if len(sys.argv) != 2 and len(sys.argv) != 3:
		print 'Usage: compile_mapped_null_profile.py <subdir> <alt_pam>'
	else:
		subdir = sys.argv[1]
                alt_pam = eval(sys.argv[2]) if len(sys.argv) > 2 else False
		...
		for fasta_file in fasta_files:
			...
			pam_lookup = shortenLookupIds(loadPamLookup(getPamLocFile(),adj=pam_adj))
			exp_oligo_lookup = loadExpOligoLookup(subdir)
